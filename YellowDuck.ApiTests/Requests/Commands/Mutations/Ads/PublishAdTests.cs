...
{
    public class PublishAdTests : TestBase
    {
        private readonly PublishAd handler;
        private readonly AppUser user;
        private readonly Ad ad;

        public PublishAdTests()
        {
            handler = new PublishAd(DbContext, NullLogger<PublishAd>.Instance);
        ...
        private PublishAd.Input GetValidInput()
        {
            return new PublishAd.Input
            {
        ...
        {
            var input = new PublishAd.Input
            {
                AdId = "QWQ6MB=="
            };

            await F(() => handler.Handle(input, CancellationToken.None))
                .Should().ThrowAsync<PublishAd.AdNotFoundException>();
        }
    }
}
