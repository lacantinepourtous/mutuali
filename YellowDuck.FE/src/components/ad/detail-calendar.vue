<template>
  <div class="availability">
    ...
    <div class="availability__grid">
      <div class="availability__calendar">
        <b-calendar
          ...
          @input="(date) => updatePeriodCheckboxes(date)"
        ></b-calendar>
      </div>

      <div v-if="activeCalendarDate && !viewOnly" class="availability__period">
        <p class="availability__period-title font-weight-bold mb-2">
          <span v-if="locale === 'fr-CA'" class="text-capitalize">{{ $format.dateDay(calendarContext.selectedDate) }},</span>
          {{ $format.dateLong(calendarContext.selectedDate) }}
        </p>
        <fieldset class="availability__period-fieldset">
          <legend class="sr-only">{{ $t("aria-label.calendar-choose-period") }}</legend>
          <b-checkbox
            v-if="checkPeriodAvailability(calendarContext.selectedYMD, calendarContext.selectedDate, DAY)"
            ref="dayCheckbox"
            class="availability__period-checkbox"
            :value="DAY"
            v-model="dayCheckboxValue"
            @input="(v) => updateSelectedDates(v, calendarContext.selectedDate, DAY)"
          >
          ...
          <b-checkbox
            v-if="checkPeriodAvailability(calendarContext.selectedYMD, calendarContext.selectedDate, EVENING)"
            ref="eveningCheckbox"
            class="availability__period-checkbox"
            :value="EVENING"
            v-model="eveningCheckboxValue"
            @input="(v) => updateSelectedDates(v, calendarContext.selectedDate, EVENING)"
          >
    ...
    <div v-if="selectedDates.length > 0" class="availability__selection mt-4">
      <p class="font-weight-bold">{{ $t("label.calendar-selection") }}</p>
      <ul class="availability__selection-list list-unstyled">
        <li v-for="(selectedDate, index) in selectedDates" :key="index" class="availability__selection-item">
          <span v-if="$format.dateShortDay(selectedDate.date)" class="text-capitalize">
            {{ $format.dateShortDay(selectedDate.date) }},
          </span>
          <span>{{ $format.dateShort(selectedDate.date) }}</span>
          <span>-</span>
          <span>
            <span v-if="selectedDate.day">{{ $t("label.ad-dayAvailability") }} </span>
            <span v-if="selectedDate.day && selectedDate.evening">{{ $t("text.and") }} </span>
            <span v-if="selectedDate.evening" :class="{ 'text-lowercase': selectedDate.day }">
              {{ $t("label.ad-eveningAvailability") }}
            </span>
          </span>
  ...
  computed: {
    ...
    formattedRestrictions() {
      return this.restrictions.map((restriction) => {
        return {
          startDate: this.$format.date(restriction.startDate, "YYYY-MM-DD"),
          day: restriction.day,
    ...
    autoMessage: function () {
      if (this.selectedDates && this.selectedDates.length > 0) {
        ...
        const formattedDates = this.selectedDates.map((date) => {
          let periods = [];
          if (date.day) periods.push(this.$t("label.ad-dayAvailability").toLowerCase());
          if (date.evening) periods.push(this.$t("label.ad-eveningAvailability").toLowerCase());
          return `${this.$format.dateLong(date.date)} (${periods.join(" " + this.$t("text.and") + " ")})`;
        });
        datesString = formattedDates.join(", ");
        return this.$t("btn.create-conversation-auto-message-with-dates", { dates: datesString });
      } else {
        return this.$t("btn.create-conversation-auto-message");
      }
    }
  },
  methods: {
    ...
    checkPeriodAvailability(ymd, date, period) {
      if (!date) return false;
      const dayIndex = date.getDay() === 0 ? 6 : date.getDay() - 1;
      const dayValue = this.availabilityWeekdayOptions[dayIndex].value;
      // Check if the date & the period are in the restrictions
      if (this.formattedRestrictions.length && this.formattedRestrictions.some((restriction) => restriction.startDate === ymd)) {
        const restriction = this.formattedRestrictions.find((restriction) => restriction.startDate === ymd);
    ...
    addAvailableDateClasses(ymd, date) {
      let classes = "";
      if (this.checkPeriodAvailability(ymd, date, DAY)) {
        classes += "available-day";
      }
      if (this.checkPeriodAvailability(ymd, date, EVENING)) {
    ...
    setDisabledDates(ymd, date) {
      ...
      if (this.formattedRestrictions.length && this.formattedRestrictions.some((restriction) => restriction.startDate === ymd)) {
        const restriction = this.formattedRestrictions.find((restriction) => restriction.startDate === ymd);
        if (restriction.day && restriction.evening) {
          return true;
        }
        if (
          restriction.day &&
    ...
    updateSelectedDates(selection, date, period) {
      const entryIndex = this.selectedDates.findIndex(
        (x) => this.$format.date(x.date, "YYYY-MM-DD") === this.$format.date(date, "YYYY-MM-DD")
      );
      ...
      if (selection) {
        ...
        } else if (this.selectedDates[entryIndex][period] === false) {
          this.selectedDates[entryIndex][period] = true;
        }
      } else {
        if (entryIndex === -1) return;
        else if (this.selectedDates[entryIndex].day && this.selectedDates[entryIndex].evening) {
          this.selectedDates[entryIndex][period] = false;
        } else if (this.selectedDates[entryIndex][period] === true) {
          this.selectedDates.splice(entryIndex, 1);
        }
      }
      this.$emit("update-conversation-message", this.autoMessage);
    },
    updatePeriodCheckboxes(date) {
      const entryIndex = this.selectedDates.findIndex(
        (x) => this.$format.date(x.date, "YYYY-MM-DD") === this.$format.date(date, "YYYY-MM-DD")
      );
      if (entryIndex === -1) {
        this.dayCheckboxValue = "";
        this.eveningCheckboxValue = "";
      } else {
        this.dayCheckboxValue = this.selectedDates[entryIndex].day ? DAY : "";
        this.eveningCheckboxValue = this.selectedDates[entryIndex].evening ? EVENING : "";
      }
    },
    removeDate(date) {
      ...
      if (this.$format.date(this.activeCalendarDate, "YYYY-MM-DD") === this.$format.date(date, "YYYY-MM-DD")) {
        this.updatePeriodCheckboxes(date);
      }
      this.$emit("update-conversation-message", this.autoMessage);
    }
  }
